...
struct alignas(8) Reply {
    ...
    int64_t bank = 0, wallet = 0, bank_limit = limit, wallet_limit = limit;
...
inline bool valid_reply(const Reply& state) {
    if (state.magic_value != magic || state.protocol != version || state.length != sizeof(Reply) ||
        state.flags || state.reserved || state.result > SaveFailed ||
        state.bank_limit != limit || state.wallet_limit != limit ||
        state.bank < 0 || state.bank > limit || state.wallet < 0 || state.wallet > limit ||
        state.max_deposit < 0 || state.max_deposit > std::min(state.wallet, limit - state.bank) ||
        state.max_withdraw < 0 || state.max_withdraw > std::min(state.bank, limit - state.wallet)) return false;
    for (int i = 0; i < 2; ++i)
        if (state.buy[i] != buy_prices[i] || state.sell[i] != sell_prices[i] ||
            ...
            state.max_sell[i] > static_cast<uint64_t>((limit - state.bank) / sell_prices[i])) return false;
    return true;
}
...
inline Plan plan(const Reply& state, uint32_t action, int64_t amount) {
    ...
    if (state.bank < 0 || state.bank > limit || state.wallet < 0 || state.wallet > limit || amount <= 0)
        return out;
    if (action == Deposit) {
        if (amount > state.wallet) { out.result = Funds; return out; }
        if (amount > limit - state.bank) { out.result = Limit; return out; }
        out.bank += amount; out.wallet -= amount;
    } else if (action == Withdraw) {
        if (amount > state.bank) { out.result = Funds; return out; }
        if (amount > limit - state.wallet) { out.result = Limit; return out; }
        out.bank -= amount; out.wallet += amount;
        ...
        if (amount > INT32_MAX || amount > limit / price) { out.result = Limit; return out; }
        ...
        if (buy) {
            ...
            if (total > limit - state.bank) { out.result = Limit; return out; }
            out.bank += total; out.item_delta = -static_cast<int32_t>(amount);
        }
    } else return out;
    out.result = Ok;
