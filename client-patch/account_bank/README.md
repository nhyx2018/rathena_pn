...
The translated base omits item `12781`. `SystemEN/AccountBankInfo.lua` now supplies a complete ticket definition before applying bank descriptions, using existing coupon artwork while preserving resources on installations that already define the item. Install the **v2.2 ItemFix** patch and fully restart the game to correct **Unknown Item** in existing inventories. The panel version remains v2.2. See the [ticket repair and actual-loader regression](../../doc/bank_ticket_20260913.md).
