...
class Client:
    ...
    def bytes(self,action,amount,sequence=None):
        ...
        return struct.pack('<IHHIIIIQQQqII',0x314b4250,1,64,AID,self.cid,self.key1,self.key2,
    ...
    def send(self,data,fragment=False):
        ...
        assert struct.unpack_from('<IHH',response)==(0x314b4250,1,136)
...
def start_map():
    log=Path('/app/rathena-builds/bank-20260913/runtime/runtime-map.log')
...
def main():
    ...
    for action,value,code in ((1,-1,3),(1,9223372036854775807,6),(3,5,8),(4,2,9)):
    ...
    c.close()
    print(json.dumps({'passed':True,'cases':cases,'final_state':before,'synthetic_account':AID,
