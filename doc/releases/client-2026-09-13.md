> **Latest bank patch: v2.2 ItemFix.** After extracting the full client, install `PN-Bank-v2.2-ItemFix-20260913.zip` from the same release. It includes the stable v2.2 panel and fixes **1M Zeny Ticket** appearing as **Unknown Item**. Close the game, replace `BankUI.dll`, and merge the patch's `SystemEN` folder into the client folder, replacing `AccountBankInfo.lua`. Restart the game; the panel still shows **v2.2**. This supersedes both earlier bank patches. See the [ticket repair](../bank_ticket_20260913.md) and [refresh verification](../bank_refresh_20260913.md).
...
| `PN-Bank-v2.2-ItemFix-20260913.zip` | Cumulative bank panel and ticket metadata update; apply after extraction |
...
3. Apply **`PN-Bank-v2.2-ItemFix-20260913.zip`** as described above. Its two replacement client files supersede those in the original full-client manifest; the patch includes its own SHA-256 checksums.
4. Open `PN-Client`, run **`Check Client.cmd`**, adjust display settings in **`Setup.exe`**, then launch **`Start Game.cmd`**.
