...
The Mystic Box menu provides Storage I–X, Master Storage I–VI, Guild Storage, Card Storage, Character Bound Storage, Rename Storages, Reorder Storages and Expand Storages. Open it with `@storage`, `@mstorage`, the **Mystic Box at Prontera (158, 185)** beside the Main Office entrance, or the Main Office Kafra.

To unlock a page, choose **Expand Storages**, now the first menu option, and select a locked page. You can also select a **red page directly**. Both routes show the page name, its 600-slot capacity, account sharing and the 50,000,000-zeny wallet cost before asking for confirmation. Opening or cancelling the confirmation does not charge zeny.
...
**17 authenticated native storage scenarios passed** through actual login, character and map processes. Coverage includes every menu entry, initial locks, both player commands, GM level 99, clicking the visible Mystic Box, direct locked-page confirmation, cancellation without payment, normal/premium deposits and withdrawals, card/bound filters, names and ordering, cross-character persistence, the 50M fee, insufficient funds, disconnect/retry recovery, carts, invalid packets, guild access rejection, a full 600-slot page and committed/uncommitted transfers across real map crashes.
