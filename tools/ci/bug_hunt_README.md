...
`report.json`, `dashboard.md`, and `identity.json`. Identity contains current
...
the selected offline checks passed on stable server inputs, not that rendered
gameplay, SQL crash durability, or the installed production build was verified.
Client hashes describe the initial inventory; keep the client files stable during
the run. Reports retain source hashes even when the working tree is dirty.
